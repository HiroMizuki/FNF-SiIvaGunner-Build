...
class PlayState extends MusicBeatState
{
	...
	var arcadeCityLights:FlxTypedGroup<BGSprite>;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				scatman = new BGSprite('scatman/scatman', -500, -100, 0.9, 0.9);
				add(scatman);
				scatman.alpha = 0;
			...
			case 'tyler': //Week 3 - Pico
				var bg:BGSprite = new BGSprite('tyler/stageback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('tyler/stagefront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);

			case 'louvre': //Week 3 - Pico (In-Game Version)
				GameOverSubstate.characterName = 'bf-dead-art-hip';

				var bg:BGSprite = new BGSprite('louvre/stageback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('louvre/stagefront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);

			case 'palace': //Week 1 - Fresh (Poop Version)
			...
			case 'bus': //Week 1 - Fresh (Itch.io Build)
				...
				var bg:BGSprite = new BGSprite('bus/stageback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('bus/stagefront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
				if(!ClientPrefs.lowQuality) {
			...
			case 'angryBirds': //Week 1 - Bopeebo (Newgrounds Build)
				var bg:BGSprite = new BGSprite('angrybirds/birdsback', -600, -200, 0.9, 0.9);
				add(bg);

				if(!ClientPrefs.lowQuality) {
					var stageFront:BGSprite = new BGSprite('angrybirds/birdsfront', -650, 600, 0.9, 0.9);
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					add(stageFront);

					var stageCurtains:BGSprite = new BGSprite('angrybirds/birdscurtains', -500, -300, 1.3, 1.3);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.9));
					stageCurtains.updateHitbox();
					add(stageCurtains);
				}
			...
			case 'pigInAWig': //Week 1 - Bopeebo (In-Game Version)
				var bg:BGSprite = new BGSprite('pig/pigback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('pig/pigfront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
			...
			case 'spooky': //Week 2
				var thefolder:String = '';

				if (Paths.formatToSongPath(SONG.song) == 'monster-(in-game-version)') {
					thefolder = 'gray/';
					GameOverSubstate.characterName = 'bf-gray-dead';
					GameOverSubstate.loopSoundName = 'gameOver-variationsonacloud';
					GameOverSubstate.endSoundName = 'gameOverEnd-variationsonacloud';
				}

				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite(thefolder + 'halloween_bg', -200, -100, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite(thefolder + 'halloween_bg_low', -200, -100);
				}
				add(halloweenBG);
			...
			case 'philly': //Week 3
				...
				if (Paths.formatToSongPath(SONG.song) == 'fresh-(ost-version)')
					thefolder = 'subwaySurfers/';

				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('philly/' + thefolder + 'sky', -100, 0, 0.1, 0.1);
					add(bg);
				}

				var city:BGSprite = new BGSprite('philly/' + thefolder + 'city', -10, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				add(city);

				phillyLightsColors = [0xFF31A2FD, 0xFF31FD8C, 0xFFFB33F5, 0xFFFD4531, 0xFFFBA633];
				phillyWindow = new BGSprite('philly/' + thefolder + 'window', city.x, city.y, 0.3, 0.3);
				...
				if(!ClientPrefs.lowQuality) {
					var streetBehind:BGSprite = new BGSprite('philly/' + thefolder + 'behindTrain', -40, 50);
					add(streetBehind);
				}
			...
			case 'beach': //Week 6 - Senpai
				GameOverSubstate.deathSoundName = 'fnf_loss_sfx-pixel';
				GameOverSubstate.loopSoundName = 'gameOver-pixel';
				GameOverSubstate.endSoundName = 'gameOverEnd-pixel';
				GameOverSubstate.characterName = 'bf-pixel-dead';

				var bgSky:BGSprite = new BGSprite('weeb/weebMountains', 0, 0, 0.1, 0.1);
				add(bgSky);
				bgSky.antialiasing = false;

				var repositionShit = -200;

				var bgSchool:BGSprite = new BGSprite('weeb/weebBeach', repositionShit, 0, 0.6, 0.90);
				add(bgSchool);
				bgSchool.antialiasing = false;

				var bgStreet:BGSprite = new BGSprite('weeb/weebSidewalk', repositionShit, 0, 0.95, 0.95);
				add(bgStreet);
				bgStreet.antialiasing = false;
			...
			case 'ballisticAlley' | 'alley':
				switch (Paths.formatToSongPath(SONG.song)) {
					default:
						wBg = new BGSprite('whittyBack', -600, -200);
					case 'lo-fight':
						wBg = new BGSprite('whittyAltBack', -600, -200);
					case 'ballistic':
						wBg = new BGSprite('miamiBack', -600, -200);
					case 'overhead':
						wBg = new BGSprite('marsBack', -600, -200);
				}
				...
				if(curStage == 'ballisticAlley') {
					...
					{
						case 'ballistic':
							nwBg.frames = Paths.getSparrowAtlas('BallisticMiami');
						case 'ballistic-(beta-mix)':
				...
				wstageFront = new BGSprite('whittyFront', -650, 630);
				wstageFront.setGraphicSize(Std.int(wstageFront.width * 1.1));
				add(wstageFront);

			case 'arcade':
				GameOverSubstate.characterName = 'bf-dead-kapi';
				var stageback:BGSprite = new BGSprite('FLback', -500, -200);
				add(stageback);

				var city:BGSprite = new BGSprite('city', -500, -180);
				arcadeCityLights = new FlxTypedGroup<BGSprite>();
				add(arcadeCityLights);

				for (i in 0...5)
				{
						var light:BGSprite = new BGSprite('win' + i + 'alt', city.x, -200);
						light.visible = false;
						light.updateHitbox();
						arcadeCityLights.add(light);
				}

				var stagefront:BGSprite = new BGSprite('stagefront', -600, 600);
				stagefront.setGraphicSize(Std.int(stagefront.width * 1.1));
				stagefront.updateHitbox();
				add(stagefront);

				littleGuys = new BGSprite('littleduds', 25, 200, 1, 1, ['Bottom Level Boppers instance 1']);
 				littleGuys.setGraphicSize(Std.int(littleGuys.width * 1));
				littleGuys.updateHitbox();

				if (Paths.formatToSongPath(SONG.song) == 'beathoven')
					add(littleGuys);
		}
		...
		switch(Paths.formatToSongPath(SONG.song))
		{
			...
			case 'spookeez-(beta-mix)':
				GameOverSubstate.characterName = '06-dead';
			case 'spookeez-(in-game-version)' | 'ballistic-(beta-mix)':
				GameOverSubstate.characterName = 'charles-barkley-dead';
				GameOverSubstate.loopSoundName = 'gameOver-barkley';
			case 'spookeez-(jpn-version)':
				GameOverSubstate.characterName = 'bf-sonic-dead';
			case 'dad-battle-(jp-version)':
				GameOverSubstate.characterName = 'ryuko-dead';
			case 'fresh-(vocal-mix)':
				GameOverSubstate.characterName = 'bf-cat-dead';
			case 'fresh-(poop-version)':
				GameOverSubstate.characterName = 'link-dead';
			case 'fresh':
				GameOverSubstate.characterName = 'blue-dead';
			case 'bopeebo-(itchio-build)':
				GameOverSubstate.characterName = 'bf-scat-dead';
			...
			case 'ballistic':
				GameOverSubstate.deathSoundName = 'fnf_loss_sfx-miami';
			case 'beathoven':
				GameOverSubstate.deathSoundName = 'fnf_loss_sfx-touhou';
		...
		var doofButKapi:DialogueBoxKapi = new DialogueBoxKapi(false, dialogue);
		// doof.x += 70;
		...
		doofButKapi.scrollFactor.set();
		doofButKapi.finishThing = startCountdown;
		...
		{
			funneEffect = new FlxSprite(-600, -200).loadGraphic(Paths.image('thefunnyeffect', 'bonus'));
	...
	function kapiIntro(?dialogueKapi:DialogueBoxKapi):Void
	{
		inCutscene = true;
		var black:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		black.scrollFactor.set();
	...
	function whittyAnimation(?dialogueBox:DialogueBox, bside:Bool):Void
	{
		...
		var wind:FlxSound = new FlxSound().loadEmbedded(Paths.sound('windLmao'), true);
		FlxG.sound.list.add(wind);
		var mBreak:FlxSound = new FlxSound().loadEmbedded(Paths.sound('micBreak'));
		FlxG.sound.list.add(mBreak);
		var mThrow:FlxSound = new FlxSound().loadEmbedded(Paths.sound('micCock'));
		FlxG.sound.list.add(mThrow);
		var mSlam:FlxSound = new FlxSound().loadEmbedded(Paths.sound('slammin'));
		FlxG.sound.list.add(mSlam);
		var TOE:FlxSound = new FlxSound().loadEmbedded(Paths.sound('ouchMyToe'));
		FlxG.sound.list.add(TOE);
		var soljaBOY:FlxSound = new FlxSound().loadEmbedded(Paths.sound('souljaboyCrank'));
		FlxG.sound.list.add(soljaBOY);
		...
		switch(Paths.formatToSongPath(SONG.song)) // WHITTY ANIMATION CODE LMAOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
		{
			case 'ballistic':
				var cutsceneHandler:CutsceneHandler = new CutsceneHandler();
				...
				animation.frames = Paths.getSparrowAtlas('cuttinDeezeMars', 'bonus');
				...
				add(white);
				trace(white);
				cutsceneHandler.push(animation);
				...
				var boyfriendCutscene:FlxSprite = new FlxSprite(boyfriend.x, boyfriend.y);
				boyfriendCutscene.frames = Paths.getSparrowAtlas('characters/BOYFRIEND');
				boyfriendCutscene.animation.addByPrefix('idle', 'BF idle dance', 24, true);
				boyfriendCutscene.animation.play('idle', true);
				boyfriendCutscene.animation.curAnim.finish();
				addBehindBF(boyfriendCutscene);
				cutsceneHandler.push(boyfriendCutscene);
				...
				cutsceneHandler.timer(2.166, function()
				{
					if (!mBreak.playing)
						mBreak.play();
				});

				cutsceneHandler.timer(3.583, function()
				{
					if (!mSlam.playing)
						mSlam.play();
				});

				cutsceneHandler.timer(3.625, function()
				{
					if (!mThrow.playing)
						mThrow.play();
				});

				cutsceneHandler.timer(5.125, function()
				{
					if (!rumble.playing)
						rumble.play();
				});

				cutsceneHandler.timer(5.333, function()
				{
					if (!soljaBOY.playing)
					{
						soljaBOY.play();
						...
						camFollowPos.camera.shake(0.01, 3);
					}
				});

				cutsceneHandler.timer(5.625, function()
				{
					FlxG.camera.stopFX();
				});

				cutsceneHandler.timer(6.583, function()
				{
					if (!TOE.playing)
					{
						TOE.play();
						camFollowPos.camera.stopFX();
						camFollowPos.camera.shake(0.03, 6);
					}
				});

				cutsceneHandler.timer(9.291, function()
				{
					FlxTween.tween(white, {alpha: 1}, 0.3, {onComplete: function(twn:FlxTween) {
						dad.alpha = 1;
						boyfriendGroup.alpha = 1;
						remove(animation);
						nwBg.animation.addByPrefix('gameButMove', 'Background Whitty Moving', 16, true);
				...
				cutsceneHandler.finishCallback = function()
				{
					...
					FlxTween.tween(white, {alpha: 0}, 0.8, {onComplete: function(twn:FlxTween) {
						if (dialogueBox != null)
						{
							camHUD.visible = true;
							wind.fadeOut();
			...
			case 'lo-fight':
				...
				var boyfriendCutscene:FlxSprite = new FlxSprite(boyfriend.x + 314, boyfriend.y);
				boyfriendCutscene.frames = Paths.getSparrowAtlas('characters/BOYFRIEND');
				boyfriendCutscene.animation.addByPrefix('idle', 'BF idle dance', 24, true);
				...
				camFollow.set(dad.getMidpoint().x + 40, dad.getMidpoint().y - 180);
				camHUD.visible = false;
				boyfriend.x += 314;
				...
				new FlxTimer().start(0.01, function(tmr:FlxTimer)
					{
						...
						else
						{
							...
							{
								tmr.reset(0.01);
								trace('animation at frame ' + animation.animation.curAnim.curFrame);

								switch(animation.animation.curAnim.curFrame)
								{
									case 0:
									...
									case 34:
										FlxG.sound.play(Paths.sound('rip'));
									case 147:
	...
	function cacheCountdown()
	{
		...
		introAssets.set('jam', ['ready', 'set', 'jam/go']);
		introAssets.set('pixel', ['pixelUI/ready-pixel', 'pixelUI/set-pixel', 'pixelUI/date-pixel']);

		var introAlts:Array<String> = introAssets.get('default');
		if (isPixelStage) introAlts = introAssets.get('pixel');

		if (PlayState.instance.boyfriend.curCharacter == 'charles-barkley' || PlayState.instance.dad.curCharacter.endsWith('-ballin')) 
			introAlts = introAssets.get('jam');
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (songNotes[1] > 3 && !opponentChart || songNotes[1] <= 3 && opponentChart)
				{
		...
		switch(event.event) {
			...
			case 'Philly Glow':
				...
				switch(Paths.formatToSongPath(SONG.song)) {
					default:
						win = 'window';
					case 'blammed-(br-version)':
						win = 'shadow';
				}

				phillyWindowEvent = new BGSprite('philly/' + win, phillyWindow.x, phillyWindow.y, 0.3, 0.3);
			...
			if(!inCutscene)
			{
				...
				if(startedCountdown)
				{
					...
					notes.forEachAlive(function(daNote:Note)
					{
						...
						if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote && dad.curCharacter != 'rapper-mario-dead') {
	...
	public function moveCamera(isDad:Bool)
	{
		...
		else
		{
			...
			if (Paths.formatToSongPath(SONG.song) == 'tutorial' || Paths.formatToSongPath(SONG.song) == 'tutorial-(beta-mix)' && cameraTwn == null && FlxG.camera.zoom != 1)
			{
	...
	function tweenCamIn() {
		if (Paths.formatToSongPath(SONG.song) == 'tutorial' || Paths.formatToSongPath(SONG.song) == 'tutorial-(beta-mix)' && cameraTwn == null && FlxG.camera.zoom != 1.3) {
	...
	private function cachePopUpScore()
	{
		...
		Paths.image(shitPart1 + "good" + shitPart2);
		Paths.image(shitPart1 + "bad" + shitPart2);
		Paths.image(shitPart1 + "shit" + shitPart2);
		Paths.image(shitPart1 + "combo" + shitPart2);
		
		for (i in 0...10) {
			Paths.image(shitPart1 + 'num' + i + shitPart2);
		}
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image(shitPart1 + daRating.image + shitPart2));
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(shitPart1 + 'combo' + shitPart2));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(shitPart1 + 'num' + Std.int(i) + shitPart2));
	...
	private function keyShit():Void
	{
		...
		if (!char.stunned && generatedMusic && startedCountdown)
		{
			// rewritten inputs???
			notes.forEachAlive(function(daNote:Note)
			{
	...
	{
		if (!opponentChart) {
			if (Paths.formatToSongPath(SONG.song) != 'tutorial' || Paths.formatToSongPath(SONG.song) == 'tutorial-(beta-mix)')
				camZooming = true;
		}
	...
	override function beatHit()
	{
		...
		switch (Paths.formatToSongPath(SONG.song)) //hardcoded events
		{
			case 'bopeebo-(itchio-build)':
				switch (curBeat) {
					case 96:
						FlxTween.tween(scatman, {alpha:1}, 32);
					case 164:
						FlxTween.tween(scatman, {alpha:0}, 2);
				}
			
			case 'pico-(ost-version)':
				switch (curBeat) {
					case 16:
						FlxTween.tween(black, {alpha:0}, 8);
					case 136:
		...
		switch (curStage)
		{
			...
			case 'arcade':
				...
				if (curBeat % 4 == 0)
				{
					arcadeCityLights.forEach(function(light:FlxSprite)
					{
						light.visible = false;
					});

					curLight = FlxG.random.int(0, arcadeCityLights.length - 1);

					arcadeCityLights.members[curLight].visible = true;
					// arcadeCityLights.members[curLight].alpha = 1;
				}
		}

		if (curStage == 'spooky' && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset && Paths.formatToSongPath(SONG.song) != 'south')
		{
			lightningStrikeShit();
		} else if (Paths.formatToSongPath(SONG.song) == 'south') {
			switch (curBeat) {
				case 40, 72, 104, 136, 168, 200, 232:
					lightningStrikeShit();

					if (curBeat == 200)
						halloweenBG.alpha = 0.4;
